...
size_t insertion_idx_(const std::vector<T>& elements, const C& val, size_t l, size_t r){
    size_t mid = (l+r)/2;
    if(l==r) return (elements[l]<val)? r+1:r; //edge cases
    if(elements[mid] == val) return mid;
    if(elements[mid] < val) return insertion_idx_(elements, val, mid, r);
    else return insertion_idx_(elements, val, l, mid);
}
...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    // if(elements[elements.size()]<val) return elements.size();
    // return insertion_idx_(elements, val, 0, elements.size()-1);
